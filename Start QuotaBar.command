...
if [[ ! -x "$BIN" || "$SHA" != "$OLD" ]]; then
  osascript -e 'display notification "正在编译 QuotaBar 1.8.8…" with title "QuotaBar"'
...
osascript -e 'display notification "已出现在屏幕右上角 · 系统盘 D · 外接盘各一只 E · 插拔即更新" with title "QuotaBar 1.8.8"' || true
