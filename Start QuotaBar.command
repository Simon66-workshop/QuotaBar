...
if [[ ! -x "$BIN" || "$SHA" != "$OLD" ]]; then
  osascript -e 'display notification "正在编译 QuotaBar 1.3.2…" with title "QuotaBar"'
...
osascript -e 'display notification "已出现在屏幕右上角菜单栏 · 左键面板 · 右键菜单" with title "QuotaBar 1.3.2"' || true
